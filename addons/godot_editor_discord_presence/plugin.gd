...
func _on_reconnect_timer_timeout():
	...
	if rpc and is_instance_valid(rpc):
		if not rpc.is_connected_to_client() and rpc.status != DiscordRPC2.CONNECTING:
...
func _on_rpc_error(err) -> void:
	if typeof(err) == TYPE_INT and err == DiscordRPC2.ERR_CLIENT_NOT_FOUND:
